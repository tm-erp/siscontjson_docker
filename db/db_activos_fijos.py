...
def getCategoryAF(conn: Connection, name:str):
        ...
        """
        query = f"""SELECT [NomGrupCodigo] AS code
                          ,[NomgrupDescripcion] AS title  
        ...
        df["asset_category_name"] = [f'{row['code']} {row['title']}' for index, row in df.iterrows()]
        #Elimino las columnas code_init y code_last
        df.drop(['code', 'title'], axis=1, inplace=True)
        ...
        df["finance_books"] = [[{ "finance_book": f'{row['codeGroup']}-{row['titleGroup']}',
                                 "depreciation_method": f'{"Straight Line" if row["tipo_depresiacion"]==1 else "Manual" }',
                                   "total_number_of_depreciations": (100/row["tasa"] if row['tasa']>0 else 0 )* 12,
        ...
        df["accounts"] = [[{"fixed_asset_account": f'240{row['codeGroup']}', 
                                   "accumulated_depreciation_account": f'375{row['codeGroup']}'}] for index, row in df.iterrows()]
        
        
        #Elimino las columnas code_init y code_last
        df.drop(['codeGroup', 'codeSubGroup', 'title', 'titleGroup', 'tasa', 'tipo_depresiacion'], axis=1, inplace=True)
...
def getFinanceBook(conn: Connection, name:str):
        ...
        #Genero los numeros de los subelementos
        df["finance_book_name"] = [f'{row['code']}-{row['title']}' for index, row in df.iterrows()]
        #Elimino las columnas code_init y code_last
        df.drop(['code', 'title'], axis=1, inplace=True)
